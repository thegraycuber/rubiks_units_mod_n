...
def insert_prime_powers(n, n_index, n_prime_powers, curr_alg, n_algs, curr_unit):
    ...
    for curr_multiplier in range(n_prime_powers[n_index][1]):
        if n_index == 0:
            print("Main Cycle ", curr_multiplier, " of ", len(n_prime_powers))
        n_algs = insert_prime_powers(
            n,
            n_index + 1,
            n_prime_powers,
            curr_alg + n_prime_powers[n_index][5][curr_multiplier],
            n_algs,
            curr_unit,
        )
...
def compress(n_algs, symbol_index, new_symbols):
    ...
    for unit in n_algs:
        ...
        for start_symbol in range(len(unit[1]) - 1):
            freq_tracker = freq_insert(
                freq_tracker, unit[1][start_symbol], unit[1][start_symbol + 1], b
            )
    ...
    for sym in new_symbols:
        ...
        compression.append(
            [sym, symbol_index[max_split[0]] + symbol_index[max_split[1]]]
        )
        for unit in n_algs:
            for start_symbol in range(len(unit[1]) - 2, -1, -1):
                if (
                    unit[1][start_symbol] == max_split[0]
                    and unit[1][start_symbol + 1] == max_split[1]
                ):
...
new_symbols = ["A", "a", "C", "c", "E", "e", "G", "g", "H", "h", "I", "i", "J",
               "j", "K", "k", "M", "m", "N", "n", "O", "o", "P", "p", "Q", "q",
               "S", "s", "T", "t", "V", "v", "W", "w", "X", "x", "Y", "y", "Z",
